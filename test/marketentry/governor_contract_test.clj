  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Kita Systems SRB"}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Kita Systems SRB" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest nonresident-pib-missing-is-held-and-unoverridable
  (testing "missing verified Serbian PIB for a non-resident engagement -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:nonresident-pib-missing} (-> (store/ledger db) last :basis)))
