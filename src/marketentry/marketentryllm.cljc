(ns marketentry.marketentryllm
  ...
  the SRB public-sector market-entry compliance actor.
  ...
  cited), never a committed record or a real Portal javnih nabavki
...
(defn- propose-submit
  ...
  :actuation/submit-filing` -- real-world Portal javnih nabavki
  submission."
  [db {:keys [subject]}]
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向けポータル提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "requires-pib?=" (:requires-pib? e)
                        " pib-verified?=" (:pib-verified? e)
