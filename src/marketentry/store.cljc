(ns marketentry.store
  "SSoT for the SRB market-entry compliance actor, behind a `Store`
...
(defn demo-data
  ...
  []
  {:engagements
   {"eng-1" {:id "eng-1" :operator "Kita Systems SRB" :portal "Portal javnih nabavki"
             :base-fee 400000 :monthly-rate 25000 :monitoring-months 12
             :claimed-fee 700000.0
             :requires-pib? true :pib-verified? true
             :drafted? false :submitted? false
             :jurisdiction "SRB" :status :intake}
    "eng-2" {:id "eng-2" :operator "Atlantis LLC" :portal "Portal javnih nabavki"
             :base-fee 400000 :monthly-rate 25000 :monitoring-months 12
             :claimed-fee 700000.0
             :requires-pib? true :pib-verified? true
             :drafted? false :submitted? false
             :jurisdiction "ATL" :status :intake}
    "eng-3" {:id "eng-3" :operator "Minami Trading Beograd" :portal "Portal javnih nabavki"
             :base-fee 400000 :monthly-rate 25000 :monitoring-months 12
             :claimed-fee 999000.0
             :requires-pib? true :pib-verified? true
             :drafted? false :submitted? false
             :jurisdiction "SRB" :status :intake}
    "eng-4" {:id "eng-4" :operator "Higashi Export" :portal "Portal javnih nabavki"
             :base-fee 400000 :monthly-rate 25000 :monitoring-months 12
             :claimed-fee 700000.0
             :requires-pib? true :pib-verified? false
             :drafted? false :submitted? false
             :jurisdiction "SRB" :status :intake}}})
...
(defn- engagement->tx [{:keys [id operator portal base-fee monthly-rate monitoring-months claimed-fee
  ...
  (cond-> {:engagement/id id}
    ...
    (some? requires-pib?)                 (assoc :engagement/requires-pib? requires-pib?)
    (some? pib-verified?)                 (assoc :engagement/pib-verified? pib-verified?)
...
(def ^:private engagement-pull
  [:engagement/id :engagement/operator :engagement/portal :engagement/base-fee :engagement/monthly-rate
   ...
   :engagement/requires-pib? :engagement/pib-verified?
...
(defn- pull->engagement [m]
  (when (:engagement/id m)
    {:id (:engagement/id m) :operator (:engagement/operator m) :portal (:engagement/portal m)
     ...
     :requires-pib? (boolean (:engagement/requires-pib? m))
     :pib-verified? (boolean (:engagement/pib-verified? m))
